 ...
 * @access public
 */
class ButterBean_Review_Control_Gallery extends ButterBean_Review_Control {
    ...
    public function __construct( $manager, $name, $args = array() ) {
        ...
        $this->l10n = wp_parse_args(
            $this->l10n,
            array(
                'upload' => esc_html__('Add image', 'stm_motors_review'),
                'set' => esc_html__('Add images', 'stm_motors_review'),
                'choose' => esc_html__('Choose images', 'stm_motors_review'),
                'change' => esc_html__('Add images', 'stm_motors_review'),
                'remove' => esc_html__('Remove all', 'stm_motors_review'),
                'drop' => esc_html__('Drag and drop featured image here', 'stm_motors_review'),
                'placeholder' => esc_html__('No images selected', 'stm_motors_review')
            )
        );
    }
    ...
    public function to_json() {
        ...
        if(has_post_thumbnail($post_id) and $this->name === 'gallery') {
			if(count($value) > 1 && $value[0] != get_post_thumbnail_id($post_id)) array_unshift($value, get_post_thumbnail_id($post_id));
